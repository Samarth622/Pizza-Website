...
function Header() {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false) // Corrected variable name
  const { darkMode, toggleDarkMode } = useDarkMode()

  const toggleMenu = () => {
    setIsMenuOpen(!isMenuOpen); // Corrected variable name
  }
    ...
    <nav className={`${darkMode ? 'dark bg-gray-800' : 'light bg-black'} w-full flex justify-between items-center gap-1 lg:px-16 px-6 py-6 sticky top-0 z-50`}>
      ...
      <ul className='lg:flex justify-center items-center gap-10 hidden'>
        { navItems.map(({link, path}) => (
          <Link key={path} className='text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-orange-500 dark:hover:text-black hover:text-white' to={path} spy={true} offset={-100} smooth={true}>{link}</Link>
        )) }
      </ul>

      <div id='header-icons' className='lg:flex hidden justify-center items-center gap-8 text-white'>
        <FaSearch className='w-[22px] h-[22px] tranform hover:scale-125 transition-transform duration-300 cursor-pointer' />
        <IoPerson className='w-[22px] h-[22px] tranform hover:scale-125 transition-transform duration-300 cursor-pointer' />
        <FaHeart className='w-[22px] h-[22px] tranform hover:scale-125 transition-transform duration-300 cursor-pointer' />
      ...
      <div className={`${isMenuOpen ? 'flex' : 'hidden'} w-full h-fit bg-slate-800 p-4 absolute top-[80px] left-0`} onClick={closeMenu}>
        <ul className='flex flex-col justify-center items-center gap-2 w-full'>
          {navItems.map(({link, path}) => ( // Added parentheses around the map function
